...
public interface ServicioEntidad {
     ...
     * 
     * @param <K>
     *            Tipo del identificador del tipo {@code T}
     * @param <T>
     *            Tipo del {@code Identificable & Nombrable}
     * @param clase
     *            Tipo usado para referenciar los recursos buscados
     * @return {@code Collection<T>} que contiene los recursos buscados
     */
    default <K extends Comparable<K>, T extends Identificable<K>> Collection<T> getElementosRegistradosDe(
            Class<T> clase) {
        return getGestorNombrables().getIdentificables(clase);
    }

    /**
     * Devuelve el {@link Identificable} con el {@code id} de tipo {@code K} requerido o {@code null}. Si
     * no se encuentra el {@code id} se emitira un mensaje en {@link ServicioEntidad#SERVICIO_ENTIDAD_LOG}
     ...
     *            Tipo del {@code Identificable & Nombrable}
     * @param clase
     *            Tipo usado para referenciar el recurso buscado
     * @param id
     ...
     * agrega al {@link GestorNombrables} de este servicio. con el {@code nombre} pasado.
     * 
     * @param <T>
     *            Tipo de {@code AbstractNombrable} que se quiere crear
     * @param clase
     *            Tipo que se quiere crear
     * @param nombre
     *            Nombre que se quiere establecer
    ...
    /**
     * Genera un {@code GeneradorIdentificadores<String>} por defecto para la {@code clase} y lo almacena con
     * el resto de generadores o recupera el existente asociado a esa clase.
     * 
     * @param <T>
     *            Tipo de {@code Identificable<String>} que se quiere crear
     * @param clase
     *            Clase para la que crear el Generador
     * @return {@code GeneradorIdentificadores<String>} creado o recuperado
     */
    default <T extends Identificable<String>> GeneradorIdentificadores<String> getGeneradorIdsParaClase(
            Class<T> clase) {
        return crearGeneradorIdsParaClase(clase, false);
    }

    /**
     * Genera un {@link GeneradorIdentificadores} por defecto para la {@code clase} y lo almacena con el
     * resto de generadores o recupera el existente asociado a esa clase en funcion de si se quiere uno
     * {@code nuevo} o no.
     * 
     * @param <T>
     *            Tipo de {@code Identificable<String>} que se quiere crear
     * @param clase
     *            Clase para la que generar el Generador
     * @param nuevo
     *            si se quiere enlazar con un nuevo generador independientemente de si existe uno
     * @return {@code GeneradorIdentificadores<String>} creado
     */
    @SuppressWarnings("unchecked")
    default <T extends Identificable<String>> GeneradorIdentificadores<String> crearGeneradorIdsParaClase(
            Class<T> clase, boolean nuevo) {
